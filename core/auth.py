...
from .crypto_kdf import derive_kek
from .crypto_sym import aes_gcm_encrypt_with_key, aes_gcm_decrypt_with_key
...
def register_user(email: str, passphrase: str) -> Tuple[bool, str, str]:
    ...
    # Derivamos KEK
    kek = derive_kek(passphrase, salt, t=KDF_PARAMS["t"], m=KDF_PARAMS["m"], p=KDF_PARAMS["p"], outlen=KDF_PARAMS["outlen"])
...
def login(email: str, passphrase: str) -> Tuple[bool, str, Dict[str, Any], str]:
    ...
    salt = _unb64u(u["salt"])
    kp = u["kdf_params"]
    kek = derive_kek(passphrase, salt, t=kp["t"], m=kp["m"], p=kp["p"], outlen=kp["outlen"])
